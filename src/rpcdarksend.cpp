...
Value darksend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "darksend <GanjaProject> <amount>\n"
            "GanjaProject, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "darksend <GanjaProject> <amount>\n"
            "GanjaProject, denominate, or auto (AutoDenominate)"
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (strCommand == "genkey")
    {
        ...
        return CGanjaProjectSecret(secret).ToString();
    }
