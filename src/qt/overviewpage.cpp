...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeGanjaProjectAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeGanjaProjectAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeGanjaProjectAmount = strAnonymizeGanjaProjectAmount.remove(strAnonymizeGanjaProjectAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeGanjaProjectAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeGanjaProjectAmount*COIN) nMaxToAnonymize = nAnonymizeGanjaProjectAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeGanjaProjectAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeGanjaProjectAmount));
        strAnonymizeGanjaProjectAmount = strAnonymizeGanjaProjectAmount.remove(strAnonymizeGanjaProjectAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeGanjaProjectAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeGanjaProjectAmount == 0){
