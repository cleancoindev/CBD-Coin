    ...
    fNewRecipientAllowed(true)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a GanjaProject address (e.g. iACLambeGgkxodKtfzM7huFXyPYWjz33Z2)"));
#endif
...
bool SendCoinsDialog::handleURI(const QString &uri)
{
    ...
    {
        CGanjaProjectAddress address(rv.address.toStdString());
        if (!address.IsValid())
            return false;
        pasteEntry(rv);
        return true;
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::NarrationTooLong:
        ...
        break;
    case WalletModel::GanjaprojectFee:
        msgParams.first = tr("A fee %1 times higher than %2 per recipient is considered an ganjaprojectly high fee.").arg(10000).arg(BitcoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), MIN_RELAY_TX_FEE));
        break;
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        CGanjaProjectAddress addr = CGanjaProjectAddress(text.toStdString());
