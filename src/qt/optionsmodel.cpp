...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeGanjaProjectAmount"))
        settings.setValue("nAnonymizeGanjaProjectAmount", 1000);
    nAnonymizeGanjaProjectAmount = settings.value("nAnonymizeGanjaProjectAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeGanjaProjectAmount"))
        SoftSetArg("-anonymizeGanjaProjectamount", settings.value("nAnonymizeGanjaProjectAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeGanjaProjectAmount:
            return QVariant(nAnonymizeGanjaProjectAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeGanjaProjectAmount:
            nAnonymizeGanjaProjectAmount = value.toInt();
            settings.setValue("nAnonymizeGanjaProjectAmount", nAnonymizeGanjaProjectAmount);
            emit AnonymizeGanjaProjectAmountChanged(nAnonymizeGanjaProjectAmount);
            break;
        default:
            break;
        }
