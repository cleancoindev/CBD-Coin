...
bool WalletModel::validateAddress(const QString &address)
{
    ...
    CGanjaProjectAddress addressParsed(sAddr);
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        CScript scriptPubKey = GetScriptForDestination(CGanjaProjectAddress(rcp.address.toStdString()).Get());
...

    {
        ...
        // reject ganjaproject fee
        unsigned int ganjaprojectfee = (transaction.getTransactionSize() == 0 ? nAddresses : transaction.getTransactionSize());
        ganjaprojectfee = ((MIN_RELAY_TX_FEE * ganjaprojectfee) * 10000);
        if (nFeeRequired > ganjaprojectfee){
            LogPrintf("nFeeRequired: %d -- GanjaprojectFee: %d\n", nFeeRequired, ganjaprojectfee);
            return GanjaprojectFee;
        }
    }
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            if (rcp.typeInd == AddressTableModel::AT_Stealth)
            {
                ...
                if (sxAddr.SetEncoded(sAddr))
                {
                    ...
                    CGanjaProjectAddress addrTo(ckidTo);
            ...
            CScript scriptPubKey;
            scriptPubKey.SetDestination(CGanjaProjectAddress(sAddr).Get());
            vecSend.push_back(make_pair(scriptPubKey, rcp.amount));
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        CTxDestination dest = CGanjaProjectAddress(strAddress).Get();
    ...
    const CTxDestination &address, const std::string &label, bool isMine, ChangeType status)
{
    ...
    {
    QString strAddress = QString::fromStdString(CGanjaProjectAddress(address).ToString());
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(!out.fSpendable || !ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
        mapCoins[QString::fromStdString(CGanjaProjectAddress(address).ToString())].push_back(out);
    }
}
